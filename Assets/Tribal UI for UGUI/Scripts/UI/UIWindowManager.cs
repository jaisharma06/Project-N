		...
		protected virtual void Update()
		{
			...
			if (Input.GetKeyUp(KeyCode.Escape))
			{
				...
				foreach (UIWindow window in windows)
				{
					...
					if (window.isActiveAndEnabled && window.gameObject.activeInHierarchy && window.escapeKeyAction != UIWindow.EscapeKeyAction.None)
					{
				...
				if (EligibleForShow)
				{
					...
					foreach (UIWindow window in windows)
					{
						...
						if (window.isActiveAndEnabled && window.gameObject.activeInHierarchy && !window.IsOpen && window.escapeKeyAction == UIWindow.EscapeKeyAction.Toggle)
						{
							// Show the window
							window.Show();
						}
