	...
	public class UIStepBar : MonoBehaviour {
		...
		public void UpdateBubble()
		{
			...
			if (this.m_CurrentStep > 0 && this.m_CurrentStep <= this.m_StepsCount)
			{
				...
				GameObject stepObject = this.m_StepsGameObjects[this.m_CurrentStep - 1];
				...
				if (this.m_BubbleText != null)
					this.m_BubbleText.text = UIStepBar.DigitToRoman(this.m_CurrentStep);
			}
			else
			{
		...
		public void UpdateGridProperties()
		{
			...
			// Grid spacing
			float spacingX = Mathf.Floor((this.m_StepsGridRect.rect.width - (float)this.m_StepsGridPadding.horizontal - ((float)this.m_StepsCount * this.m_SeparatorSize.x)) / (float)(this.m_StepsCount - 1) / 2f) * 2f;
		...
		public void RebuildSteps()
		{
			...
			// Create the steps
			for (int i = 0; i < this.m_StepsCount; i++)
			{
				GameObject step = new GameObject("Step " + i.ToString(), typeof(RectTransform), typeof(Image));
		...
		public static string DigitToRoman(int number)
		{
			if ((number < 1) || (number > 3999))
				return string.Empty;
			
			if (number >= 1000) return "M" + DigitToRoman(number - 1000);
			if (number >= 900) return "CM" + DigitToRoman(number - 900);
			if (number >= 500) return "D" + DigitToRoman(number - 500);
			if (number >= 400) return "CD" + DigitToRoman(number - 400);
			if (number >= 100) return "C" + DigitToRoman(number - 100);            
			if (number >= 90) return "XC" + DigitToRoman(number - 90);
			if (number >= 50) return "L" + DigitToRoman(number - 50);
			if (number >= 40) return "XL" + DigitToRoman(number - 40);
			if (number >= 10) return "X" + DigitToRoman(number - 10);
			if (number >= 9) return "IX" + DigitToRoman(number - 9);
			if (number >= 5) return "V" + DigitToRoman(number - 5);
			if (number >= 4) return "IV" + DigitToRoman(number - 4);
			if (number >= 1) return "I" + DigitToRoman(number - 1);
